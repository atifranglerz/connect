...
class ChatController extends Controller
{
    public function index()
    {
        ...
        $customer = ChatFavorite::with('customer')->where([['vendor_id',Auth::id()],['vendor_status',0]])->latest('updated_at')->get();
    ...
    public function store(Request $request){
      ...
      $id = $request->id;
        ...
        $date = Carbon::now();
        $vendor = ChatFavorite::where('vendor_id',$auth_id)->first();
        $vendor->updated_at = $date;
        $vendor->save();

        $customer = ChatFavorite::where('customer_id',$id)->first();
