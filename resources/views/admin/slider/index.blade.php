                            ...
                            <form action="{{ url('admin\slider') }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="card-body">
                                    <div class="custom-file">
                                        <input type="file" class="custom-file-input" id="validatedImage" accept="image/*" name="image" required>
                                        <label class="custom-file-label" for="validatedImage">Choose Image...</label>
                                        @error('image')
                                        ...
                                        <tbody>
                                            @forelse($data as $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if(isset($item->image))
                                                            <img src="{{ asset($item->image) }}" alt="" width="100px" height="50px">
                                                        @else
                                                            Null
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <a href="{{ url('admin/slider/edit/'.$item->id) }}" class="btn btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                                                        {{-- <form action="{{ url('admin/slider/destroy/'.$item->id)}}" method="POST" style="display: inline-block">
                                                            @csrf
                                                            @method('DELETE')
                                                        ...
                                                        <a>
                                                            {{-- <i class="fas fa-trash text-danger glyphicon glyphicon-trash"
                                                             ...
                                                             <svg xmlns="http://www.w3.org/2000/svg" data-id="{{ $item->id }}" class="fas fa-trash text-danger glyphicon glyphicon-trash" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                                                             </a>
                                                            <form id="del_form{{ $item->id }}"
                                                             action="{{ url('admin/delete-slider/' . $item->id) }}">
                                                              @csrf
                                                               @method('DELETE')
                                                             </form>
                                                    </td>
