...
<head>
    ...
    .form_sending_wraper>textarea::-webkit-scrollbar,
    ul.sidebar_navcigation::-webkit-scrollbar {
        display: none;
    }
    </style>
    <style>
...
<body>
    ...
    $(document).ready(function() {
        ...
        $(".input-images>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Car image</p><input type="file" name="car_images[]" size="60" ></label>  '
            );
        $('.input-images-2').imageUploader();
        $(".input-images-2>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label skip"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Police/Accident/Inspection Report</p><a href="#" class="skip">Skip</a><input type="file" name="file" size="60" ></label>   '
            );
        $('.input-images-3').imageUploader();
        $(".input-images-3>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Add Registration Copy Image</p><input type="file" name="files[]" size="60" ></label>'
            );
        $('.input-images-4').imageUploader();
        $(".input-images-4>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload workshop image</p><input type="file" name="image" name="file" size="60" ></label>'
            );
        ...
        $(".input-images-5>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Your Picture </p><input type="file" name="pimage" size="60" ></label>'
            );
        $('.input-images-6').imageUploader();
        $(".input-images-6>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Your ID</p><input type="file" name="pid" size="60" ></label>'
            );
        $('.input-images-7').imageUploader();
        $(".input-images-7>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Your Trade License and ID </p><input type="file" size="60" ></label> '
            );
        ...
        $(".Upload_final_report>.image-uploader>.upload-text").append(
            '<label class="img_wraper_label"><div class="file_icon_wraper"><img src="{{ asset('
            public / vendor / assets / images / fileuploadicon
            .svg ') }}"></div><p class="mb-0">Upload Final Report</p><input type="file" size="60" ></label>'
            );
