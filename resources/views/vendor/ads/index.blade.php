    ...
    <section class="pb-5 login_content_wraper">
        <div class="px-4 container-lg container-fluid">
            ...
            <div class="row g-2">
                            ...
                            <div class="all_quote_card ">
                                ...
                                <div class="  w-100  quote_detail_wraper align-items-md-center">
                                    <div class="quote_info Leavereview">
                                        <h5 class="d-flex align-items-center active_quote heading-color">{{ $data->model }}
                                        </h5>
                                        <p class="mb-0">{{ __('msg.Car Maker') }} : {{ $data->company->company }}</p>
                                        <p class="mb-0">{{ __('msg.Model Year') }} : {{ $data->modelYear->model_year }}
                                        </p>
                                    ...
                                    <div class="quote_detail_btn_wraper">
                                        ...
                                        <div class="d-flex  align-items-center chat_view__detail">
                                            <form method="post" action="{{ route('vendor.ads.destroy', $data->id) }}">
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="btn-secondary edit_del_btns me-2">{{ __('msg.delete') }}</button>
                                            </form>
                                            <a href="{{ route('vendor.ads.edit', $data->id) }}"
                                                class="btn btn-primary edit_del_btns">{{ __('msg.edit') }}</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                @else
                    <div class="all_quote_card">
                        <p class="mb-0">{{ __('msg.Oops... No Ads Found related to this vendor!') }}</p>
                    </div>
                @endif
                <span>{!! $ads->links() !!}</span>
