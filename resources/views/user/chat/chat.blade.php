        ...
        <div class="main_message">
            @if (count($message) > 0)
                @foreach ($message as $data)
                                ...
                                @if ($data->msgtype == 'text')
                                    <p class="mb-0 message_txt" id="receiver_side">
                                        <span class="fa fa-trash text-danger position-absolute del-content delete"
                                            aria-hidden="true"
                                            style="right: 3px;top: 3px;font-size: 11px;cursor: pointer"
                                            id="{{ $data->id }}"></span>
                                        {{ $data->body }}
                                    </p>
                                @else
                    ...
                    @else
                        <div class="inbox_contact align-items-end justify-content-end top_main" id="{{$data->id}}delete">
                            <div class="message_txt_wraper">
    ...
    <div class="chat_top_name ">
        <div class="d-flex justify-content-between align-items-center" >
            <a href="#">
                <div class="inbox_contact top_main">
                    <?php
                    $vendor = \App\Models\User::where('id', $chated_user->id)->first();
                    $gettime = strtotime($vendor->online_status) + 5;
                    $now = strtotime(Carbon\Carbon::now());
                    ?>
                    <div class="contact_img">
        ...
        <div class="main_message">
            @if (count($message) > 0)
                @foreach ($message as $data)
                    ...
                    @if ($data->customer_sender_id != Auth::id())
                        <div class="inbox_contact align-items-end top_main" id="{{$data->id}}delete">
                            <div class="contact_img">
